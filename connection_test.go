...
func TestFlinkConn_Filesystem_RowAndMap_ScanIntoStructAndMap(t *testing.T) {
	...
	rows, err := db.QueryContext(ctx, "SELECT id, name, info FROM csv_complex")
	if err != nil {
		t.Fatalf("select failed: %v", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		var r rec
		if err := rows.Scan(&r.id, &r.name, &r.info); err != nil {
			t.Fatalf("scan failed: %v", err)
		}
		got = append(got, r)
	}
	...
	for _, r := range got {
		key := fmt.Sprintf("id=%d|name=%s|info={%s}", r.id, r.name, r.info.Canonical())
		gotSet[key] = struct{}{}
	}

	want := []rec{
		{id: 1, name: "alpha", info: RowInfo{Score: 10, Label: "A"}},
		{id: 2, name: "beta", info: RowInfo{Score: 20, Label: "B"}},
	}
