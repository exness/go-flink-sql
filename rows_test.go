...
func TestRowsNext_ReturnsErrRowsClosedWhenClosed(t *testing.T) {
	t.Run("closed flag set", func(t *testing.T) {
		cols := makeCols(false)
		r := &Rows{columns: cols, closed: true}
		dest := make([]driver.Value, len(cols))
		if err := r.Next(dest); !errors.Is(err, errRowsClosed) {
			t.Fatalf("expected errRowsClosed when Next called on closed Rows; got %v", err)
		}
	})

	t.Run("after Close() call", func(t *testing.T) {
		cols := makeCols(false)
		m := &gatewayMock{}
		fc := &flinkConn{client: m, sessionHandle: "sess"}
		r := &Rows{
		...
		dest := make([]driver.Value, len(cols))
		if err := r.Next(dest); !errors.Is(err, errRowsClosed) {
			t.Fatalf("expected errRowsClosed after Close(); got %v", err)
		}
	})
}
